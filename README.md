...
| `robots.txt` | allows everything and names the sitemap. Keep pages out of search with a `noindex` meta tag, never a `Disallow` (a blocked crawler cannot read the tag) |
| `admin/` | **Server controls** (`/admin/`), `noindex`. The game servers' control layer and the only way to reach it — there is no Discord command for any of it (Discord-Automation DR-0012). Signs in with the SAME token as the forum (one Discord session, two Workers) and talks to the portal Worker in `../Null_Ossuary/deploy/portal-worker` (`portalApiUrl` in `config.js`). **Nothing here touches a game server**: every action is queued, and Chamberlain re-checks the per-server grant before acting, so a button being drawn is never the thing that authorised it |
| `js/admin/` | ES modules for the portal: `api.js` (client; no DOM, so `node --test` runs it) and `page.js`. Same `?v=` stamp rule as the forum, checked by `tests/admin.test.mjs`, which also checks the CSP's `connect-src` matches `portalApiUrl` and that the page inserts nothing as HTML |
| `sitemap.xml` | the indexable pages (`/`, `/forums/`, `/privacy/`). Add a page here when one is added; bump its `<lastmod>` when its content changes |
